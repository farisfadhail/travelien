...
<nav class="relative px-4 py-4 flex justify-between items-center bg-white">
    ...
    <ul class="hidden absolute top-1/2 left-1/2 transform -translate-y-1/2 -translate-x-1/2 lg:flex lg:mx-auto lg:items-center lg:w-auto lg:space-x-6">
        ...
        <li>
            <a class="text-sm text-gray-400 hover:text-gray-500" href="#">About us</a>
        </li>
        @auth
            <li class="text-gray-300">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" stroke="currentColor" class="w-4 h-4 current-fill" viewBox="0 0 24 24">
    ...
    @auth
        @role('user')
            <form action="{{ route('logout') }}" method="POST">
                @csrf
                <button type="submit" class="hidden lg:inline-block lg:ml-auto lg:mr-3 py-2 px-6 bg-gray-50 hover:bg-gray-100 text-sm text-gray-900 font-bold  rounded-xl transition duration-200">Logout</button>
            </form>
        @endrole
        @role('admin')
            <form action="{{ route('logout') }}" method="POST" class="hidden lg:inline-block">
                @csrf
                <button type="submit" class="lg:ml-auto lg:mr-3 py-2 px-6 text-sm bg-gray-50 hover:bg-gray-100 text-gray-900 font-bold rounded-xl transition duration-200">Logout</button>
            </form>
        @endrole
    @else
        ...
        <a class="hidden lg:inline-block py-2 px-6 bg-blue-600 hover:bg-blue-700 text-sm text-white font-bold rounded-xl transition duration-200" href="{{ route('register') }}">Sign up</a>
    @endauth
</nav>
<div class="navbar-menu relative z-50 hidden">
    ...
    <nav class="fixed top-0 left-0 bottom-0 flex flex-col w-5/6 max-w-sm py-6 px-6 bg-white border-r overflow-y-auto">
        <div class="flex items-center mb-8">
            <a class="mr-auto text-3xl font-bold leading-none" href="{{ route('landing-page') }}">
        ...
        <div>
            <ul>
                <li class="mb-1">
                    <a
                        class="{{ request()->routeIs('landing-page') ? 'block p-4 text-sm font-semibold text-blue-600 hover:bg-blue-50 rounded' : 'block p-4 text-sm font-semibold text-gray-400 hover:bg-blue-50 hover:text-blue-600 rounded' }}"
                        href="{{ route('landing-page') }}"
                    >
                        Home
                    </a>
        ...
        <div class="mt-auto">
            <div class="pt-6">
                @auth
                    @role('admin')
                        <a class="block px-4 py-3 mb-2 leading-loose text-xs text-center text-white font-semibold bg-blue-600 hover:bg-blue-700  rounded-xl" href="{{ route('admin.dashboard') }}">Dashboard</a>
                    @endrole
                    @role('user')
                        <a class="block px-4 py-3 mb-2 leading-loose text-xs text-center text-white font-semibold bg-blue-600 hover:bg-blue-700  rounded-xl" href="{{ route('user.dashboard') }}">Dashboard</a>
                    @endrole
                @else
            ...
            <p class="my-4 text-xs text-center text-gray-400">
                <span>Copyright © {{ Date::now()->format('Y') }}</span>
            </p>
        </div>
    </nav>
</div>
